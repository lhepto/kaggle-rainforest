...
def get_image_data(no_image, directory):
    ...
    training_image_data = np.zeros((no_image, 256, 256, 4), dtype=np.uint16)
    image_iterator = 0
    for training_image in training_images:
        if (image_iterator < no_image):
            training_image_data[image_iterator, :, :, :] = (io.imread(directory+training_image))
            image_iterator += 1
...
def main():
    ...
    prediction = T.clip(lasagne.layers.get_output(network),0.001,0.999)
    ...
    test_prediction = T.clip(lasagne.layers.get_output(network, deterministic=True),0.001,0.999)
    ...
    training_fn = theano.function([input_var, target_var], [test_loss, test_acc])
    ...
    for epoch in range(num_epochs):
        ...
        err, acc = training_fn(X_train, y_train)
        print("  total training loss:\t\t\t{:.6f}".format(err))
        print("  total training acc:\t\t{:.6f}".format(acc))
